module BiTruncatedTesseract.BiTruncatedTesseract
  where
import           BiTruncatedTesseract.Data
...
display rot1 rot2 rot3 angle angle2 zoom = do
  ...
  z <- get zoom
  a <- get angle
  let points  = map (rotate4D (pi/4) (pi/4) (alpha * pi / 180)) btVertices
      ppoints = map project4D points
      vectors = map toVector3 ppoints
      edges   = map (both (toVertex3 . (!!) ppoints)) btEdges
--      facets = map (map (toVertex3 . (!!) ppoints)) octahedralFacets
  loadIdentity
  (_, size) <- get viewport
  resize z size
...
--  mapM_ (renderPrimitive Polygon . drawFacet) facets
  swapBuffers
  where
...
resize zoom s@(Size w h) = do
  ...
  lookAt (Vertex3 (-8) 6 (-20+zoom)) (Vertex3 0 0 0) (Vector3 0 1 0)
...
idle anim angle2 = do
    ...
    when a $ do
        when (r < 360) $ do
            let ppm = printf "butruncatedtess%04d.ppm" (round r :: Int)
...
main = do
  ...
  _ <- createWindow "Bitruncated tesseract"
