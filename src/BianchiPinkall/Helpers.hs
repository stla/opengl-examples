...
pinkallFun :: Int -> Double -> Double -> [Double]
pinkallFun n u v = stereom (flatTorus n u v)

pinkallFun' :: Int -> Double -> Double -> Vertex3 Double
pinkallFun' n u v = toVx3 $ pinkallFun n u v
  where
  toVx3 x = Vertex3 (x!!0) (x!!1) (x!!2)

sequ,seqv :: [Double]
sequ = [realToFrac i/100 * 2 * pi | i <- [0 .. 100]]
seqv = [realToFrac i/100 * pi | i <- [0 .. 100]]

quad :: Int -> [Double] -> [Double] -> Int -> Int
...
allQuads :: Int -> [((Vertex3 Double, Vertex3 Double, Vertex3 Double, Vertex3 Double), Normal3 Double)]
