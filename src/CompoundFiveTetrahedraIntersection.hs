module CompoundFiveTetrahedraIntersection
  where
import           CompoundFiveTetrahedra.Intersection
...
display rot1 rot2 rot3 zoom = do
  ...
  mapM_ (\v -> preservingMatrix $ do
                  ...
                  renderObject Solid $ Sphere' 0.01 30 30)
        vertices
...
resize zoom s@(Size w h) = do
  ...
  lookAt (Vertex3 0 0 (-1.5 + zoom)) (Vertex3 0 0 0) (Vector3 0 1 0)
...
keyboardAndMouse rot1 rot2 rot3 capture zoom key keyState _ _ =
  case (key, keyState) of
    ...
    (Char 'c', _) -> do
      i <- get capture
      let ppm = printf "cgal06%04d.ppm" i
          png = printf "cgal06%04d.png" i
