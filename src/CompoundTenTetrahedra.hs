...
display rot1 rot2 rot3 zoom = do
  ...
  imapM_ drawTetrahedron tetrahedra
  imapM_ (\i x -> mapM_ (drawEdge i) x) edges
  imapM_ (\i x -> mapM_ (drawVertex i) x) vertices'
  swapBuffers

drawTetrahedron :: Int -> [[Vertex3 GLfloat]] -> IO ()
drawTetrahedron i faces =
  renderPrimitive Triangles $ do
    materialDiffuse Front $= Color4 0.5 0 0.5 1 --pickColor' i 1
...
drawVertex i v =
  preservingMatrix $ do
    ...
    materialDiffuse Front $= white -- pickColor' i 0.5
...
drawEdge i (v1,v2) = do
  ...
  renderPrimitive Quads $ do
    materialDiffuse Front $= white -- pickColor' i 1
