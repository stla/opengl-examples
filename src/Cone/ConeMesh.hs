...
cmesh0 :: Floating a => a -> a -> a -> Int -> Int
       -> ([(Vector (a,a,a), Vector (a,a,a))], [(Int,Int,Int,Int)])
cmesh0 h rr r nstacks nslices = ((vertices, normals), quads)
  where
    ...
    i_ = [0 .. nstacks]
    j_ = [0 .. nslices-1]
    u_ = [i * (h'-d)/realToFrac nstacks | i <- i_]
    v_ = [j * 2 * pi / realToFrac nslices | j <- j_]
    grid = [(u,v) | u <- u_, v <- v_]
    grid' = fromList grid
    ...
    (vertices, normals) =
      V.map (\(u,v) -> let g = h' - u in
                       ...
                       let sinv = rr/h' * sinv in
                       let t1 = (-cosv, -sinv, hdivhmd) in
                       let t2 = (-g * sinv, g*cos, 0) in
