...
display rot1 rot2 rot3 angle angle2 zoom = do
  ...
  z <- get zoom
  a <- get angle
  let points  = cube10 -- map (rotate4D (pi/4) (pi/4) (alpha * pi / 180)) vs120trunc
--      ppoints = map (stereoprojectnIterated 7) points
      ppoints1 = map (\x -> stereoprojectn' x) points
      ppoints2 = map (\x -> stereoprojectn' x) ppoints1
      ppoints3 = map (\x -> stereoprojectn' x) ppoints2
      ppoints4 = map (\x -> stereoprojectn' x) ppoints3
      ppoints5 = map (\x -> stereoprojectn' x) ppoints4
      ppoints6 = map (\x -> stereoprojectn' x) ppoints5
      ppoints7 = map (\x -> stereoprojectn' x) ppoints6
      vectors = map toVector3 ppoints7
  loadIdentity
  (_, size) <- get viewport
  resize z size
  ...
  mapM_ (\vec -> preservingMatrix $ do
        ...
        vectors
  mapM_ (drawCylinder 0.05) [head cube10, last cube10]
  swapBuffers
  where
    toVector3 x = Vector3 (x!!0) (x!!1) (x!!2)
    toVertex3 x = Vertex3 (x!!0) (x!!1) (x!!2)
    drawCylinder :: GLdouble -> [Double] -> IO ()
    drawCylinder radius vs = do
      let cylinder1 = prism (toVx3 (vs!!0)) (toVx3 (vs!!1)) 30 radius
      renderPrimitive Quads $ do
        materialDiffuse FrontAndBack $= whitesmoke
        mapM_ f cylinder1
      let cylinder2 = prism (toVx3 (vs!!5)) (toVx3 (vs!!6)) 30 radius
      renderPrimitive Quads $ do
        materialDiffuse FrontAndBack $= whitesmoke
        mapM_ f cylinder2
        where
