...
display angle = do
  ...
  alpha <- get angle
  let points  = map (rotate4D (pi/2) 0 (alpha * pi / 180)) hdVertices
      ppoints = map project4D points
      vectors = map toVector3 ppoints
      edges   = map (both (toVertex3 . (!!) ppoints)) hdOtherEdges
      redEdges   = map (both (toVertex3 . (!!) ppoints)) hdFacetEdges
      ridges  = map (map (toVertex3 . (!!) ppoints)) hdFacet
  loadIdentity
  imapM_ (\i vec -> preservingMatrix $ do
                      translate vec
                      if i `elem` hdFacetVertices
                        then materialDiffuse Front $= red
                        else materialDiffuse Front $= whitesmoke
                      renderObject Solid $ Sphere' 0.2 30 30)
         vectors
  mapM_ (drawCylinder whitesmoke 0.1) edges
  mapM_ (drawCylinder red 0.1) redEdges
