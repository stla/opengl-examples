...
display angle = do
  ...
  alpha <- get angle
  let points  = map (rotate4D (pi/4) (pi/4) (alpha * pi / 180)) octaplexVertices
      ppoints = map project4D points
      vectors = map toVector3 ppoints
      edges   = map (both (toVertex3 . (!!) ppoints)) octaplexEdges
      ridges  = map (map (toVertex3 . (!!) ppoints)) octaplexFacet
  loadIdentity
